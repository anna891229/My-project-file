...
public class Yield_compare extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tohome.setOnClickListener(v -> {
            ...
            intent.setClass(Yield_compare.this, Homepage.class);
        ...
        toREhome.setOnClickListener(v -> {
            Intent intent = new Intent();
            intent.setClass(Yield_compare.this, Yield_Homepage.class);
            startActivity(intent);
        });
    }
    private Runnable mutiThread = new Runnable() {
        ...
        public void run() {
            ...
            try {
                URL url = new URL("https://8e3d-163-14-44-237.jp.ngrok.io/forecast/yield_compare.php?"+"dist1="+dist1+"&dist2="+dist2+"&dist3="+dist3);
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    for (int i = 0; i<22; i++) {                            //建立區域1 lineChart的數值
                        if(i%2==0 && i<20){
                            value1.add(new Entry(count, Float.parseFloat(c[i])));
                            count++;
                        }
                        if(i==18 || i==20){
                            forecast1.add(new Entry(count-1,Float.parseFloat(c[i])));
                            count++;
                        }
                    }
                    ...
                    for (int i = 22; i<44; i++) {                            //建立區域2 lineChart的數值
                        if(i%2==0 && i<42){
                            value2.add(new Entry(count2, Float.parseFloat(c[i])));
                            count2++;
                        }
                        if(i==40 ||i==42){
                            forecast2.add(new Entry(count2-1,Float.parseFloat(c[i])));
                            count2++;
                        }
                    }
                    ...
                    for (int i = 44; i<65; i++) {                            //建立區域3 lineChart的數值
                        if(i%2==0 && i<64){
                            value3.add(new Entry(count3, Float.parseFloat(c[i])));
                            count3++;
                        }
                        if(i==62||i==64){
                            forecast3.add(new Entry(count3-1,Float.parseFloat(c[i])));
                            count3++;
                        }
                    }
                    // 未來預測圖表
