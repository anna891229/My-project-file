...
public class R_Dist_SpecificPage2 extends AppCompatActivity {
    EditText chooseroad;
    TextView chooseDist;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_rdist_specific_page2);
        context = this;
        dbtype= "rent"; // database name
        ...
        tohome.setOnClickListener(v -> {
            ...
            intent.setClass(R_Dist_SpecificPage2.this, Homepage.class);
            startActivity(intent);
        });

        toREhome = findViewById(R.id.Rehome);   // 跳回R首頁
        toREhome.setOnClickListener(v -> {
            Intent intent = new Intent();
            intent.setClass(R_Dist_SpecificPage2.this, R_Homepage.class);
            startActivity(intent);
        });
        ...
        public void run() {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    for (int i = 0; i < 20; i++) {                            //建立lineChart的數值
                        if(i%2==0){
                            values.add(new Entry(count, Integer.parseInt(c[i])));
                            count++;
                        }
                    }
                    // 未來預測圖表
