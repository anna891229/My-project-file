<?php
	...
	$sql2 = "SELECT `date`,COUNT(`time`) as count FROM `c_order` GROUP BY `date` ORDER BY `date`;";
	...
	for ($i=0;$i<$record;$i++) {        
		...
		array_push($label,$row2['date']);
		array_push($data,$row2['count']);
	}
	$datapoint=array();
	...
	require_once("connect.php");
	header("Content-Type: text/html; charset=utf-8");
	$sql = "SELECT * FROM `item` WHERE `amounts`<=30";
	// 用mysqli_query方法執行(sql語法)將結果存在變數中
